...
juce::AudioProcessorValueTreeState::ParameterLayout VPM2AudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>(
        ...
        juce::StringArray{ "10 ms", "100 ms", "1000 ms" },
        1 // default index (100 ms)
    ));
...
void VPM2AudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    // float maxDelayMs = *apvts.getRawParameterValue("MAX_DELAY_MS"); // commented out after changing the max delay selector mode.
