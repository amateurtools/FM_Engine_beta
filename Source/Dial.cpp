...
void Dial::setAngle(float newAngle)
{
    ...
    if (!steppedValues.empty())
    {
        ...
        for (int i = 1; i < (int)steppedValues.size(); ++i)
        {
            float dist = std::abs(value - steppedValues[i]);
            if (dist < minDist)
            {
                minDist = dist;
                closestStep = steppedValues[i];
        ...
        // Update currentStepIndex to match snapped value
        currentStepIndex = closestIndex;
...
// old triangle drawing method, may cause direct2d to be forced in windows builds which is not great for yabridge
...
//     // Triangle pointer math (same as before)
//     float pointerLength = radius - 5.0f;
//     float triangleSize = 20.0f;
//     juce::Point<float> tip =
//         center + juce::Point<float>(std::cos(angle) * pointerLength,
...
//     g.drawLine(tip.x, tip.y, base1.x, base1.y, 2.0f);
//     g.drawLine(base1.x, base1.y, base2.x, base2.y, 2.0f);
//     g.drawLine(base2.x, base2.y, tip.x, tip.y, 2.0f);
// }
